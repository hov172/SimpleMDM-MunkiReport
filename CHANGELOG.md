...
- Fixed Safari-only elastic-bounce "shake" when scrolling the MCP Findings widget's findings list, and the SimpleMDM Devices Table widget's row list, past their top/bottom on trackpad. `overscroll-behavior: contain` does not suppress Safari's own bounce animation for the scrolled element (only Chrome respects it fully here), and a brief attempt to add it was reverted after it turned out to also suppress click events on the MCP Findings widget's expand/collapse controls in Safari. A `wheel`-only `preventDefault()` fix was also insufficient on its own, since Safari can ignore `preventDefault()` once a trackpad gesture enters its momentum/inertial phase. Both widgets' scroll containers (`#simplemdm-mcp-findings-groups`, `.simplemdm-devices-table-scroll`) now pair a `wheel` listener (catches the active-finger phase) with a `scroll` listener that clamps `scrollTop` back in-bounds on every event (catches the momentum/bounce phase), without touching `overscroll-behavior` or affecting other widgets.
