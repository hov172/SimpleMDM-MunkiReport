...
### Security
- **Secret disclosure in `get_config` is now fail-closed.** The masking in `get_config()` was a deny-list of four names — `api_key`, `webhook_secret`, `action_api_secret`, `client_reporter_secret` — sitting about 150 lines from the ~48-key allow-list in `save_config()` that decides what an admin can store. Any setting added to one and not the other was returned in full, and `get_config` answers on `index?op=get_config`, a route that by design carries no session: the sync runner authenticates with the shared API key alone. So the failure mode was that a future credential setting became readable by every holder of that one key, silently. No key was leaking at the time of the change — the four that existed were all listed — this closes the shape of the bug rather than an instance of it.
  - Replaced by `Simplemdm_config_policy`, a framework-free class holding the whole policy: `SECRET_NAME_PATTERN` matches credential-shaped names, `NON_SECRET_SUFFIXES` excuses `..._enabled` feature flags, and `EXEMPT_KEYS` carries the one documented exception (`client_reporter_device_token_metadata_json`, which reports `has_token` as a boolean and never the stored hash). An unrecognised setting is treated as a secret.
  - The pattern matches `key` as a whole underscore-delimited segment rather than as the literal names `api_key`/`private_key`. A first cut did the latter, which would have disclosed a future `signing_key` or `hmac_key` — the same fail-open shape the change exists to remove. Segment matching also leaves `client_reporter_allowed_fact_keys_json` alone, which is an allow-list of permitted field names the sync runner reads, not key material.
  - Applied as a single sweep immediately before `jsonView()`, not per-key inside the settings loop. `get_config()` merges in defaults, runner configuration and derived run state *after* that loop, and a per-source check would have let all of those through.
  - Output is unchanged for both caller classes: global admins still receive every value, and non-global callers still receive exactly the four `..._set` presence flags they received before. Verified live against the running instance — of 92 stored settings, 89 are returned verbatim, 3 are reported as presence flags, and none are dropped.
  - Covered by `tests/Unit/ConfigSecretPolicyTest.php` (14 behavioural cases, including that an unlisted future secret redacts by default) and two guards in `SecretHandlingGuardTest`: one pinning the sweep to the last statement before the response, one parsing `save_config()`'s key list and failing if any credential-shaped writable setting is disclosed without a documented exemption. Both were confirmed to fail when the fix is reverted.
...
- SEC-003 (business-unit scoping) verified live, closing the last scope gap the audit had left open. The instance could not test it as-is — `reportdata` held a single machine, and there were no business units, machine groups or non-admin accounts — so a transactional harness was written instead: `tests/manual/sec003_business_unit_scoping.php` boots the real application, enables business units in-process, builds a fixture estate (two serials in the caller's group, one in another, one with no `reportdata` row at all) inside a transaction, drives the real controller as a non-admin session, and always rolls back. 21 assertions, all passing, zero residue.
  - Confirmed in particular the three exemptions the design depends on: sync-token callers and global admins bypass scoping, and with business units disabled an MCP-only device with no `reportdata` row stays visible — the regression that unconditional scoping would have caused.
  - Documented as `docs/TESTING.md` section 14 so the check is repeatable rather than a one-off.
- SEC-015 formally accepted by the maintainer, closing the audit's last outstanding item. `get_dashboard_trend` serves pre-aggregated estate-wide totals; a snapshot row holds counts, not serials, so machine-group scoping has nothing to filter on. Fixing it would mean adding `machine_group` to `simplemdm_dashboard_snapshot`, rewriting snapshot generation, and deciding what to do with historical rows that carry no group attribution — a migration, and a maintainer call rather than an audit finding. The exposure is aggregate-only (device counts and FileVault/supervision/DEP ratios; no serial, device name or per-device record) and inert unless business units are enabled. Recorded with the two conditions that would reopen it: a tenant-isolating deployment, or snapshots gaining per-device detail. Disable the trend widget if whole-estate counts are sensitive; nothing else depends on it.
