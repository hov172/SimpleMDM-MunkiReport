...
- Fixed Safari-only elastic-bounce "shake" when scrolling the MCP Findings widget's findings list past its top/bottom on trackpad. `overscroll-behavior: contain` does not suppress Safari's own bounce animation for the scrolled element (only Chrome respects it fully here), and a brief attempt to add it was reverted after it turned out to also suppress click events on the widget's expand/collapse controls in Safari (both the per-category toggle and the whole-widget minimize button stopped responding). Fixed instead with a `wheel` listener scoped to the MCP Findings widget's own list (`#simplemdm-mcp-findings-groups`) that `preventDefault()`s only wheel deltas that would scroll past the boundary, stopping Safari's bounce without touching `overscroll-behavior` or affecting other `.simplemdm-list-scroll` widgets.
