...
Ingest, read, and admin-action behavior for MCP findings can be tuned via six admin settings (`mcp_findings_enabled`, `mcp_findings_metadata_max_bytes`, `mcp_findings_auto_resolve`, `mcp_findings_event_enabled`, `mcp_findings_event_warning_threshold`, and `mcp_findings_retention_days`) — see the "MCP Findings Settings" panel in the module's admin UI, or `docs/API_REFERENCE.md` for the full effect of each. The event_enabled/event_warning_threshold settings gate an optional, deduplicated fleet findings summary event (PRD section 13): off by default, so existing installs' Events UI does not change without opt-in; when enabled, it writes under module key `simplemdm_mcp_findings_summary`, anchored to the worst-affected device. `mcp_findings_retention_days` controls how long resolved/ignored/suppressed findings are kept after they were last seen (0 = forever); ingest deletes older ones but never touches active findings.
