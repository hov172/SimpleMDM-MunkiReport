...
| `tests/Unit/CsvInjectionTest.php` | `Simplemdm_mcp_finding_model::neutralizeCsvField()` — formula triggers (`=`, `+`, `-`, `@`, tab, CR, and leading whitespace before one) get quoted, ordinary finding text is untouched, plus a guard that `export_mcp_findings()` routes every cell through it. No DB required. |
| `tests/Unit/OutputEscapingGuardTest.php` | Tripwire over the module's XSS defences: the canonical `window.simplemdmEscapeHtml` exists and escapes quotes (not just angle brackets), no view reverts to the quote-unsafe `$('<div>').text(v).html()` helper, both DataTables listings give every column a render callback, and the device listing never interpolates a raw `data` value. |
| `tests/Unit/SecretHandlingGuardTest.php` | Tripwire over secret handling: no `--api-key` on any command line, `SIMPLEMDM_API_KEY` passed via the child environment, runner stdout/stderr redacted, cron entry sources a `0600` env file written under `umask 077`, `--env-file` refuses a path inside the module, `action_secret` never read from `$_GET`, archive exclusion list intact, client-reporter integrity defaults still on, throttle wired to every shared-secret endpoint. |
| `tests/Unit/MachineScopeGuardTest.php` | Tripwire over business-unit scoping: the four scoping helpers exist, scoping stays gated on `enable_business_units` (otherwise MCP-only devices with no `reportdata` row vanish for every non-admin), global admins and sync-token callers stay exempt, every serial-scoped endpoint calls `require_serial_access()`, list/export queries stay scoped, and machine group ids are cast to int before reaching raw SQL. |
